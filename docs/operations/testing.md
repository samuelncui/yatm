...
- Run [mounted Volume and LTFS file-backend E2E](e2e-test.md) for affected copy/publication behavior. Use the official LTFS file backend on an isolated Linux test host; Volume tests use isolated already-mounted directories.
...
Alpha 1 establishes the [published data baseline](../architecture/persistence.md#published-data-formats). Preserve its forward migration/import path alongside legacy compatibility. Before the first stable v1 release, remove the [unpublished-Draft exception](../README.md#temporary-v1-draft-compatibility-policy) and publish the stable API/upgrade policy.
