...
This prototype export included Sources, exclusions and online positions from a consistent metadata view. The [persistence contract](../architecture/persistence.md#published-data-formats) now owns supported format identities and the legacy reader. An online import group requires its corresponding Source, Position and File records with validated references; old numeric IDs are not evidence of identity. Imports roll back entirely on conflicts, including across batches. Imported roots are Unconfirmed. Source revisions are local, and imported Job links are cleared because Library exports do not contain Job bundles.
...
Additional acceptance covers whole-disk/subdirectory roots, duplicates/nesting, nonexistent exclusions and changes, file/directory type transitions, frozen Archive paths and opaque identity binding, stale File-bound download URLs, HEAD/Range and active-content safety, Preview default/explicit/deduplicated/error-independent behavior, metadata round trips, legacy replacement with reused File IDs, and maintenance conflicts. Demo must exercise available, unavailable, Unconfirmed, online-only, archived, overwritten, and retryable states through normal APIs and an explicit reset smoke test.
